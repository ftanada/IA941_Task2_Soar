...
public class SoarBridge
{
    ...
    private void CreateIntegerWME(Identifier id, String s, long value) {
        SymbolFactory sf = agent.getSymbols();
    ...
    private void prepareInputLink() 
    {
        ...
        try {
            if (agent != null)
            {
              ...
              // FMT 11/04/2017 bag handling
              Bag myBag = c.getBag();
              creatureBag = CreateIdWME(creature, "BAG");
              int iTotalGems = myBag.getTotalNumberCrystals();
              Identifier gems = CreateIdWME(creatureBag,"GEMS");
              CreateIntegerWME(gems,"TOTAL",iTotalGems);
              if (iTotalGems > 0) {
                System.out.println("Bag for input link: " + myBag.printBag());
                CreateIntegerWME(gems,"Red",myBag.getNumberCrystalPerType("Red"));
                CreateIntegerWME(gems,"Green",myBag.getNumberCrystalPerType("Green"));
                CreateIntegerWME(gems,"Blue",myBag.getNumberCrystalPerType("Blue"));
                CreateIntegerWME(gems,"Yellow",myBag.getNumberCrystalPerType("Yellow"));
                CreateIntegerWME(gems,"Magenta",myBag.getNumberCrystalPerType("Magenta"));
                CreateIntegerWME(gems,"White",myBag.getNumberCrystalPerType("White"));
              }
              else {
                CreateIntegerWME(gems,"Red",0);
                CreateIntegerWME(gems,"Green",0);
                CreateIntegerWME(gems,"Blue",0);
                CreateIntegerWME(gems,"Yellow",0);
                CreateIntegerWME(gems,"Magenta",0);
                CreateIntegerWME(gems,"White",0);
              }
              int iTotalFood = myBag.getTotalNumberFood();
              CreateIntegerWME(creatureBag,"TOTAL_FOOD",iTotalFood);
              if (iTotalFood > 0) {
                CreateStringWME(creatureBag,"FOOD",myBag.printBag());
              }
              else {
                  CreateStringWME(creatureBag,"FOOD","EMPTY");
              }
              // FMT 11/04/2017 leaflet handling
              creatureLeaflet = CreateIdWME(creature, "LEAFLET");
              List<Leaflet> myLeaflets = c.getLeaflets();
              if (myLeaflets.isEmpty()) {
                CreateIntegerWME(creatureLeaflet,"TOTAL",0);
                CreateIntegerWME(creatureLeaflet,"Red",0);
                CreateIntegerWME(creatureLeaflet,"Green",0);
                CreateIntegerWME(creatureLeaflet,"Blue",0);
                CreateIntegerWME(creatureLeaflet,"Yellow",0);
                CreateIntegerWME(creatureLeaflet,"Magenta",0);
                CreateIntegerWME(creatureLeaflet,"White",0);
              }
              else {
                Integer i = 0;
                Integer red = 0;
                ...
                Integer magenta = 0;
                Integer white = 0;
                for (Iterator iter = myLeaflets.iterator(); iter.hasNext();) {
                  Leaflet leafletItem = (Leaflet) iter.next();
                  i = leafletItem.getMissingNumberOfType("Red");
                  if (i > 0) {
                      red += i;
                  }
                  i = leafletItem.getMissingNumberOfType("Green");
                  if (i > 0) {
                      green += i;
                  }
                  i = leafletItem.getMissingNumberOfType("Blue");
                  if (i > 0) {
                      blue += i;
                  }
                  i = leafletItem.getMissingNumberOfType("Yellow");
                  if (i > 0) {
                      yellow += i;
                  }
                  i = leafletItem.getMissingNumberOfType("Magenta");
                  if (i > 0) {
                      magenta += i;
                  }
                  i = leafletItem.getMissingNumberOfType("White");
                  if (i > 0) {
                      white += i;
                  }
                  //HashMap<String, Integer> myCollection = leafletItem.getWhatToCollect();
                  //for (Iterator iterMap = myCollection.keySet().iterator(); iterMap.hasNext();) { 
                  //  String item = (String)iterMap.next();
                  //  CreateIntegerWME(creatureLeaflet,item,1);
                  //  i += 1;
                  //}
                }
                CreateIntegerWME(creatureLeaflet,"Red",red);
                CreateIntegerWME(creatureLeaflet,"Green",green);
                CreateIntegerWME(creatureLeaflet,"Blue",blue);
                CreateIntegerWME(creatureLeaflet,"Yellow",yellow);
                CreateIntegerWME(creatureLeaflet,"Magenta",magenta);
                CreateIntegerWME(creatureLeaflet,"White",white);
                CreateIntegerWME(creatureLeaflet,"TOTAL",red+green+blue+yellow+magenta+white);
              }
            }
        }
        catch (Exception e)
                ...
                for (Wme com : Commands)
                {
                    ...
                    switch(commandType)
                    {
                        ...
                        case HIDE:
                            ...
                            {
                                thingNameToGet = GetParameterValue("Name");
                                if (thingNameToGet != null) commandHide.setThingName(thingNameToGet);
                                commandList.add(command);
                            }
                            break;
    ...
    public void step() throws CommandExecException
    {
        ...
        leaflet_target_string = stringLeafletTarget();
        //printInputWMEs();
        runSOAR();
    ...
    private void processGetCommand(CommandGet soarCommandGet) throws CommandExecException
    {
     ...
     /**
     * FMT 17/04/2017 Send Hide Command to World Server
    ...
    // FMT 14/04/2017 - new method for new window
    public String stringLeafletTarget() {
        Identifier il = agent.getInputOutput().getInputLink();
        String out = stringWME(il);
        return(out);
    }
